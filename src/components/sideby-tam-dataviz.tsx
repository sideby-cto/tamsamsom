  ...
  return (
    <Card className="overflow-hidden rounded-[28px] border-0 bg-[#FBF6E3] shadow-[0_8px_30px_rgba(64,22,18,0.10)]">
      <CardContent className="p-0">
        ...
        <div className="p-5">
          <div
            className="text-4xl font-black leading-none text-[#401612] md:text-5xl"
            style={{ fontFamily: '"Cabin Condensed", sans-serif' }}
          >
            {value}
          </div>
          <div
            className="mt-2 text-sm text-[#401612]/70"
...
export default function SidebyTamDataviz() {
  ...
  const assumptions = useMemo(() => {
    ...
    const usCombinedRevenue = usTeacherRevenue + usPrincipalRevenue;
  ...
  return (
    <div
      className="min-h-screen bg-[#FBF6E3] text-[#401612]"
      style={{ fontFamily: '"Bricolage Grotesque", sans-serif' }}
    >
      <div className="mx-auto max-w-7xl p-6 md:p-10">
        <div className="relative overflow-hidden rounded-[36px] bg-[#1A40F4] p-8 text-[#FBF6E3] shadow-[0_12px_40px_rgba(26,64,244,0.25)] md:p-12">
          <div
            ...
            style={{
              backgroundImage:
                "linear-gradient(rgba(251,246,227,0.35) 1px, transparent 1px)",
          ...
          <div className="relative grid items-end gap-8 md:grid-cols-[1.3fr_0.7fr]">
            <div>
              ...
              <h1
                className="text-5xl font-bold leading-[0.9] md:text-7xl"
                style={{ fontFamily: '"Cabin Condensed", sans-serif' }}
              >
                sideby market map
              </h1>
              <p className="mt-4 max-w-2xl text-base text-[#FBF6E3]/90 md:text-lg">
                A branded view of how a principal-led wedge in Washington can
                expand into teachers, a multi-state consortium, and a national
                educator learning market.
              </p>
            </div>

            <Card className="rounded-[28px] border-0 bg-[#FBF6E3] text-[#401612] shadow-none">
              <CardContent className="p-6">
                <div className="text-sm uppercase tracking-wide text-[#401612]/60">
                ...
                <div
                  ...
                  style={{ fontFamily: '"Cabin Condensed", sans-serif' }}
                >
                  {fmtMoney(assumptions.annualPrice)}
                </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          <Card className="rounded-[28px] border-0 bg-white/60 shadow-none">
            <CardHeader>
              ...
              <CardDescription>
                Adjust the annual contract value assumption.
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <Slider
                ...
                max={40}
                step={1}
                onValueChange={(values) => setMonthlyPrice(values[0] ?? 20)}
              />
            </CardContent>
          </Card>
          ...
          <Card className="rounded-[28px] border-0 bg-white/60 shadow-none">
            <CardHeader>
              ...
              <CardDescription>
                Model school-level expansion from leader to staff.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="mb-3 flex items-center justify-between text-sm">
              ...
              <Slider
                value={[teachersPerPrincipal]}
                min={10}
                max={50}
                step={1}
                onValueChange={(values) =>
                  setTeachersPerPrincipal(values[0] ?? 30)
                }
              />
            </CardContent>
          </Card>
          ...
          <Card className="rounded-[28px] border-0 bg-white/60 shadow-none">
            <CardHeader>
              ...
              <CardDescription>
                Test the effect of the regional network thesis.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="mb-3 flex items-center justify-between text-sm">
              ...
              <Slider
                value={[consortiumStates]}
                min={1}
                max={20}
                step={1}
                onValueChange={(values) => setConsortiumStates(values[0] ?? 12)}
              />
            </CardContent>
          </Card>
        </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <StatCard
            title="WA principals"
            value={fmtShort(assumptions.waPrincipalRevenue)}
            subtitle={`${assumptions.principalsWA.toLocaleString()} principals at ${fmtMoney(assumptions.annualPrice)}/year`}
            icon={School}
            tone="blue"
          />
          <StatCard
            title="WA teachers"
            value={fmtShort(assumptions.waTeacherRevenue)}
            subtitle={`${assumptions.teachersWA.toLocaleString()} teachers via principal-led expansion`}
            icon={Users}
            tone="orange"
          />
          <StatCard
            title={`${consortiumStates}-state model`}
            value={fmtShort(assumptions.consortiumRevenue)}
            subtitle="Regional expansion through consortium distribution"
            icon={Map}
            tone="pink"
          />
          <StatCard
            title="U.S. TAM"
            value={fmtShort(assumptions.usCombinedRevenue)}
            subtitle="National educator and school leader opportunity"
            icon={Network}
            tone="brown"
          />
        </div>

        <div className="mt-6 rounded-[24px] border border-[#401612]/10 bg-white/70 p-4 text-xs text-[#401612]/70">
          Assumptions: WA principals (3,800), $20/mo baseline pricing,
          principal-led expansion to teachers, and modeled multi-state
          consortium growth. Figures are bottom-up estimates for investor
          discussion.
        </div>
      </div>
    </div>
